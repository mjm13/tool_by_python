...
**阶梯式等待**：
- 检测到操作频繁时，等待时间会逐步增加（5秒 → 10秒 → 15秒...）
- 最高等待时间：30秒（取消喜欢）/ 15秒（添加歌曲）

**动态延迟调整**：
- 遇到限流：基础延迟自动增加（+0.5秒，最高5秒）
- 连续成功：延迟逐步降低（-30%，最低到配置值）

**配置建议**：
- 歌曲少于100首：`request_delay = 0.5`（默认）
- 歌曲100-500首：`request_delay = 1.0`
- 歌曲超过500首：`request_delay = 1.5-2.0`

详见：[API_RATE_LIMIT_OPTIMIZATION.md](API_RATE_LIMIT_OPTIMIZATION.md)
